...
def comparaçao(img):
    original_image=cv2.imread('divisaopintada.png')

    azul={'azul':{'B': {'max': 255 , 'min': 220 },
            'G': {'max': 0 , 'min': 0 },
            'R': {'max': 50 , 'min':0 }}}
    vermelho={'vermelho':{'B': {'max': 0 , 'min': 0 },
            'G': {'max': 0 , 'min': 0 },
            'R': {'max': 255 , 'min':240 }}}
    verde={'verde':{'B': {'max': 30 , 'min': 0 },
            'G': {'max': 255 , 'min': 245 },
            'R': {'max': 10 , 'min': 0 }}}
    amarelo={'amarelo':{'B': {'max': 55 , 'min': 0 },
            'G': {'max': 255 , 'min': 220 },
            'R': {'max': 255 , 'min':220 }}}        
    ...
    amarelo_tresh = amarelo['amarelo']

    lvlmax_azul = np.array([azul_tresh['B']['max'], azul_tresh['G']['max'], azul_tresh['R']['max']])
    lvlmin_azul = np.array([azul_tresh['B']['min'], azul_tresh['G']['min'], azul_tresh['R']['min']])

    lvlmax_vermelho = np.array([vermelho_tresh['B']['max'], vermelho_tresh['G']['max'], vermelho_tresh['R']['max']])
    ...
    if np.sum(vermelhototal)==0:
        percvermelho=0
    else:
        v=np.sum(vermelho1)
        vt=np.sum(vermelhototal)
        percvermelho=v/vt

    if np.sum(verdetotal)==0:
        percverde=0
    else:
        ver=np.sum(verde1)
        vert=np.sum(verdetotal)
        percverde=ver/vert

    if np.sum(amarelototal)==0:
        percamarelo=0
    else:
        amar=np.sum(amarelo1)
        amart=np.sum(amarelototal)
        percamarelo=amar/amart
...
def mouseCallback(event, x, y, flags, userdata, options):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if options['is_drawing']:
            cv2.line(options['pinguim_image'], (x_mouse, y_mouse), (x, y), options['pencil_color'], thickness)
            x_mouse = x
            y_mouse = y
...
def normal_mode():
    ...
    print("Press 's' to start drawing a rectangle\n")
    print("Press 'e' to start drawing a circle.\n")
    print("------------------------------------------------------\n")
    ...
    while True:
        ...
        elif pressed_key == ord('y'): # change color to yellow
            clr = (0,255,255)
...
def usp_mode():
    ...
    print("Press 's' to start drawing a rectangle\n")
    print("Press 'e' to start drawing a circle.\n")
    print("Press 'm' to switch between Mouse Mode and Camera Mode\n")
    print("------------------------------------------------------\n")
    ...
    options = {'is_drawing' : False,'pinguim_image' : blank_image,
    ...
    while True:
        ...
        if mouse_drawing == False:
            ...
            options['pinguim_image'] = blank_image
            options['pencil_color'] = clr

            cv2.setMouseCallback(window_name4, partial(mouseCallback, options=options))
        ...
        elif pressed_key == ord('m'): # increase pencil line size
            mouse_drawing = not mouse_drawing
            print('mouse drawing is now ' + str(mouse_drawing))
...
def numbered_paint():
    ...
    window_name2 = "Segmented"
    window_name3 = "Mask Largest component"
    window_name4 = "Divisão"
    blank_image = cv2.imread('divisaonumerada.png')
    #blank_image = cv2.imread("white_image.png", cv2.IMREAD_COLOR)
    thickness=3
    clr=(0,255,255)
    centroides=[]
    ...
    while True:
        ...
        if pressed_key == ord('q'):
            ...
            centroides=[] #reset no centroides usados para desenhar os traços
            blank_image = cv2.imread('divisaonumerada.png') #reset na imagem .
            print("The canvas is clean.")
            pass
